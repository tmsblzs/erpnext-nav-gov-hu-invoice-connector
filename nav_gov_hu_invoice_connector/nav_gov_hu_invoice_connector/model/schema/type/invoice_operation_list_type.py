...
from nav_gov_hu_invoice_connector.nav_gov_hu_invoice_connector.model.schema.type.invoice_operation_type import \
...
class InvoiceOperationListType(GeneratedsSuper):
    """InvoiceOperationListType -- A kéréshez tartozó kötegelt számlaműveletek
    Batch invoice operations of the request
    compressedContent -- Tömörített tartalom jelzése a feldolgozási folyamat számára
            Compressed content indicator for the processing flow
    invoiceOperation -- A kéréshez tartozó számlaművelet
    ...
    def export(self, outfile, level, namespaceprefix_='',
               namespacedef_='xmlns:tns="http://schemas.nav.gov.hu/NTCA/1.0/common" xmlns:xs="http://www.w3.org/2001/XMLSchema"  xmlns:None="http://schemas.nav.gov.hu/OSA/3.0/metrics" ',
               name_='InvoiceOperationListType', pretty_print=True):
        ...
        if self._hasContent():
            ...
            self._exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='InvoiceOperationListType',
    ...
    def _exportAttributes(self, outfile, level, already_processed, namespaceprefix_='',
                          name_='InvoiceOperationListType'):
        pass

    def _exportChildren(self, outfile, level, namespaceprefix_='',
                        namespacedef_='xmlns:tns="http://schemas.nav.gov.hu/NTCA/1.0/common" xmlns:xs="http://www.w3.org/2001/XMLSchema"  xmlns:None="http://schemas.nav.gov.hu/OSA/3.0/metrics" ',
                        name_='InvoiceOperationListType', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.compressedContent is not None:
            namespaceprefix_ = self.compressedContent_nsprefix_ + ':' if (
                        UseCapturedNS_ and self.compressedContent_nsprefix_) else ''
            showIndent(outfile, level, pretty_print)
            outfile.write('<%scompressedContent>%s</%scompressedContent>%s' % (
            namespaceprefix_, self.gds_format_boolean(self.compressedContent, input_name='compressedContent'),
        ...
        for invoiceOperation_ in self.invoiceOperation:
            ...
            invoiceOperation_.export(outfile, level, namespaceprefix_, namespacedef_='', name_='invoiceOperation',
                                     pretty_print=pretty_print)
    ...
    def to_etree(self, parent_element=None, name_='InvoiceOperationListType', mapping_=None, nsmap_=None):
        ...
        if self.compressedContent is not None:
            ...
            etree_.SubElement(element,
                              '{http://schemas.nav.gov.hu/OSA/3.0/api}compressedContent').text = self.gds_format_boolean(
                compressedContent_)
        for invoiceOperation_ in self.invoiceOperation:
