...
class QueryInvoiceCheckResponseType(BasicOnlineInvoiceResponseType):
    """QueryInvoiceCheckResponseType -- A POST /queryInvoiceCheck REST operáció válasz típusa
    Response type of the POST /queryInvoiceCheck REST operation
    invoiceCheckResult -- Jelöli, ha a lekérdezett számlaszám érvényesként szerepel a rendszerben
    és a lekérdező adószáma kiállítóként vagy eladóként szerepel a számlán
    Indicates whether the queried invoice number exists in the system as a valid invoice,
    if the tax number of the querying entity is present on the invoice either as supplier or customer
