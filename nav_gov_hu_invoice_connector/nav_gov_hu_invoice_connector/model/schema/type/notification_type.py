from nav_gov_hu_invoice_connector.nav_gov_hu_invoice_connector.model.online_invoice import GeneratedsSuper, \
    CurrentSubclassModule_, getSubclassFromModule_, Validate_simpletypes_, encode_str_2_3, UseCapturedNS_, showIndent, \
    quote_xml, ModulenotfoundExp_, GenerateDSNamespaceDefs_, quote_python, SaveElementTreeNode, Tag_pattern_
...
class NotificationType(GeneratedsSuper):
    """NotificationType -- Értesítés
    Notification
    notificationCode -- Értesítés kód
                Notification code
    notificationText -- Értesítés szöveg
                Notification text
    ...
    def export(self, outfile, level, namespaceprefix_='',
               ...
               name_='NotificationType', pretty_print=True):
        ...
        if self._hasContent():
            ...
            self._exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='NotificationType',
    ...
    def _exportChildren(self, outfile, level, namespaceprefix_='',
                        namespacedef_='xmlns:tns="http://schemas.nav.gov.hu/NTCA/1.0/common" xmlns:None="http://schemas.nav.gov.hu/OSA/3.0/metrics" ',
                        name_='NotificationType', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.notificationCode is not None:
            namespaceprefix_ = self.notificationCode_nsprefix_ + ':' if (
                        UseCapturedNS_ and self.notificationCode_nsprefix_) else ''
            showIndent(outfile, level, pretty_print)
            outfile.write('<%snotificationCode>%s</%snotificationCode>%s' % (namespaceprefix_, self.gds_encode(
                self.gds_format_string(quote_xml(self.notificationCode), input_name='notificationCode')),
                                                                             namespaceprefix_, eol_))
        if self.notificationText is not None:
            namespaceprefix_ = self.notificationText_nsprefix_ + ':' if (
                        UseCapturedNS_ and self.notificationText_nsprefix_) else ''
            showIndent(outfile, level, pretty_print)
            outfile.write('<%snotificationText>%s</%snotificationText>%s' % (namespaceprefix_, self.gds_encode(
                self.gds_format_string(quote_xml(self.notificationText), input_name='notificationText')),
                                                                             namespaceprefix_, eol_))
    ...
    def to_etree(self, parent_element=None, name_='NotificationType', mapping_=None, nsmap_=None):
        ...
        if self.notificationCode is not None:
            ...
            etree_.SubElement(element,
                              '{http://schemas.nav.gov.hu/NTCA/1.0/common}notificationCode').text = self.gds_format_string(
                notificationCode_)
        if self.notificationText is not None:
            notificationText_ = self.notificationText
            etree_.SubElement(element,
                              '{http://schemas.nav.gov.hu/NTCA/1.0/common}notificationText').text = self.gds_format_string(
                notificationText_)
